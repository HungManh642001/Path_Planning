...
def make_scenario(seed, mode='free'):
    ...
    while True:
        start = (random.uniform(width * 0.1, width * 0.9),
                 random.uniform(height * 0.1, height * 0.9))
        goal = (random.uniform(width * 0.1, width * 0.9),
                random.uniform(height * 0.1, height * 0.9))
        if 200000 < su.distance(start, goal) < 280000:
            break
    heading_start_to_goal = su.angle_to_heading(start, goal)
    topology = random.choices(['random', 'center_cluster', 'wall_block'],
                              weights=[0.2, 0.3, 0.5])[0]
    cfg = {
        'map_bounds': map_bounds,
        'start': start,
        'start_heading': heading_start_to_goal + random.uniform(-math.pi / 2, math.pi / 2),
        'goal': goal,
        'goal_heading': None,
    ...
    if mode == 'fixed':
        ...
        cfg['goal_heading'] = heading_start_to_goal + \
            random.Random(seed + 1000000).uniform(-math.pi / 2, math.pi / 2)
...
def run(args):
    ...
    for seed in seeds:
        ...
        if args.verbose:
            r = results[str(seed)]
            print(f"  seed {seed:4d}  {'OK ' if r['success'] else 'FAIL'} "
...
def _summary(payload):
    ...
    print(f"{payload['planner']:>4} [{payload['mode']}] "
...
def compare(args):
    ...
    print(f"solved: {sum(ra[k]['success'] for k in keys)} -> "
    ...
    if both:
        ...
        print(f"length:     {la / 1000:.1f}km -> {lb / 1000:.1f}km  ({100 * (lb - la) / la:+.4f}%)")
        print(f"waypoints:  {wa} -> {wb}")
        print(f"iterations: {ia} -> {ib}  ({100 * (ib - ia) / ia:+.2f}%)")
    ta, tb = a['total_time_s'], b['total_time_s']
    print(f"time:       {ta:.1f}s -> {tb:.1f}s  ({100 * (tb - ta) / ta:+.2f}%)"
