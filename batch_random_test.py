...
def generate_random_scenario(seed=42):
    ...
    while True:
        ...
        goal = (random.uniform(width * 0.1, width * 0.9), random.uniform(height * 0.1, height * 0.9))
    ...
    topology = random.choices(['random', 'center_cluster', 'wall_block'], weights=[0.1, 0.45, 0.45])[0]  # Randomly choose a topology for obstacle placement

    return mg.create_scenario({
        'map_bounds': map_bounds,
        'start': start,
        'start_heading': heading_start_to_goal + random.uniform(-math.pi/2, math.pi/2),  # Add some randomness to the start heading
        'goal': goal,
        'goal_heading': heading_start_to_goal + random.uniform(-math.pi/2, math.pi/2),  # Add some randomness to the goal heading
...
def run_batch_random_tests(num_tests=1000, output_dir="results1"):
    ...
    for i in range(num_tests):
        ...
        try:
            result = run_scenario(generate_random_scenario, scenario_name, seed=seed, output_dir=output_dir)
            all_results.append(result)
            
            if result.get('metrics'):
                perf_metrics.add_result(result['metrics'])
        except Exception as e:
            logger.exception(f"Scenario {i+1}: {scenario_name} failed with error: {e}")
